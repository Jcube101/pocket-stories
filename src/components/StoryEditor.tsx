...
export function StoryEditor({ story, setStoryStatus, onLoadStoryByPath }: Props) {
  useEffect(() => {
    if (!story) return;
  ...
  }, [story, setStoryStatus, onLoadStoryByPath]);
  ...
  return (
    <div id="editor-mode" className="mode">
      <div id="sidebar">
        ...
        <section className="sidebar-section"><h2>Tools</h2><button id="branching-script">View as Branching Script</button><button id="export-yaml">Export story.yaml</button><button id="validate-story">Validate Story</button><button id="play-story">Play Story</button></section>
