...
export function StoryEditor({ story, activeStoryId, setStoryStatus, onLoadStoryByPath }: Props) {
  ...
  return (
    <div id="editor-mode" className="mode">
      ...
      <div id="graph-container" className={isSidebarCollapsed ? 'expanded' : ''}><div id="canvas-wrapper"><div id="nodes-container"></div><svg id="svg-canvas"></svg></div><aside id="node-inspector" aria-live="polite"><h2>Node Inspector</h2><p className="inspector-empty">Select a node to inspect full details.</p><div className="inspector-content hidden"><label htmlFor="inspector-node-id">Passage ID</label><input id="inspector-node-id" type="text"/><label htmlFor="inspector-node-type">Node Type</label><select id="inspector-node-type" defaultValue="__auto__"><option value="__auto__">Auto (infer from structure)</option><option value="start">Start</option><option value="dialogue">Dialogue</option><option value="choice">Choice</option><option value="condition">Condition</option><option value="merge">Merge</option><option value="ending">Ending</option><option value="__custom__">Custom…</option></select><input id="inspector-node-type-custom" type="text" className="hidden" placeholder="Custom type token"/><label htmlFor="inspector-node-text">Full Text</label><textarea id="inspector-node-text" rows={10}></textarea><div id="inspector-node-meta"></div></div></aside></div>
    </div>
  );
}
